...
const urlHandler = {
  ...
  getFileName(name, withRenderedSuffix) {
    ...
    if ((name.indexOf('viewall-') !== -1) && (name.indexOf('viewall-') === 0) && (fileName !== '')) {
      ...
      if (withRenderedSuffix) {
        const fileSuffixRendered = ((window.config.outputFileSuffixes !== undefined) && (window.config.outputFileSuffixes.rendered !== undefined)) ? window.config.outputFileSuffixes.rendered : '';
  ...
  pushPattern(pattern, givenPath) {
    ...
    if (givenPath !== expectedPath) {
      ...
      const obj = JSON.stringify({ event: 'patternLab.updatePath', path: fileName });
      document.getElementById('sg-viewport').contentWindow.postMessage(obj, urlHandler.targetOrigin);
    } else {
      // add to the history
      const addressReplacement = (window.location.protocol === 'file:') ? null : `${window.location.protocol}//${window.location.host}${window.location.pathname.replace('index.html', '')}?p=${pattern}`;
  ...
  popPattern(e) {
    ...
    const obj = JSON.stringify({ event: 'patternLab.updatePath', path: iFramePath });
    document.getElementById('sg-viewport').contentWindow.postMessage(obj, urlHandler.targetOrigin);
